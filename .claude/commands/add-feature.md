---
description: 計画外の機能追加を .steering/ で管理し、tasklist.md 完全消化まで自律実装する
---
...
**注意:** このコマンドは `/run-phase` のフェーズベースワークフローとは独立して動作します。Builder/Validator パターンは適用しません。成果物は `.steering/` に格納されます（`outputs/` ではありません）。
...
## ステップ2: プロジェクト理解と知識ベース活用

1. `CLAUDE.md` を読み、プロジェクトの全体像を把握する。
2. `docs/` ディレクトリ内のドキュメントを確認し、関連する設計思想やアーキテクチャを理解する。
3. SDD 知識ベースを活用してコンテキストを収集する:
   - `python3 scripts/generate_context.py` を実行し、過去プロジェクトの類似コンポーネントを検索する。
   - `python3 scripts/search_knowledge.py [機能名に関連するキーワード]` でキーワード検索を行う。
   - 検索結果から再利用可能なパターンや知見があれば、計画に反映する。

## ステップ3: 既存パターンの調査

1. Grep ツールを使用し、機能名に関連するキーワードでソースコードを検索する。
2. Glob ツールを使用し、関連するファイル構造を確認する。
3. 検索結果を分析し、既存の実装パターン、命名規則、コンポーネントの利用方法を特定する。

## ステップ4: 計画フェーズ (ステアリングファイルの自動生成)

1. `Skill('add-feature')` を**計画モード**で実行し、ステップ1で作成した3つのファイル (`requirements.md`, `design.md`, `tasklist.md`) の内容を生成する。
   - テンプレート `.claude/skills/add-feature/templates/` を参照する。
   - ステップ2・3で収集したコンテキストを反映する。
...
3. タスクの実行:
  ...
  - `Skill('add-feature')` を**実装モード**で使用する。
...
1. プロジェクトの技術スタック（`docs/tech-stack.md` 参照）に応じて、適切なテストコマンドを実行する。
   - テストフレームワークが設定されている場合: テストを実行
   - リンターが設定されている場合: リントチェックを実行
   - 型チェックが設定されている場合: 型チェックを実行
...
1. `Skill('add-feature')` を**振り返りモード**で実行し、`tasklist.md` に申し送り事項を記載する。
  - 実装完了日
  - 計画と実績の差分
  - 学んだこと
  - 次回への改善提案
...
3. 影響がある場合、`docs/` 内の関連するドキュメントを `Edit` ツールで更新する。
...
- ステップ6: 品質チェックをパスする。
- ステップ7: テストがエラーなく成功する（テストが設定されている場合）。
