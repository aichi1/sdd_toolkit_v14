...
| 症状 | 原因 | 対処法 | 再開ポイント |
|------|------|--------|------------|
| Web検索で情報が見つからない | 対象が新しすぎる、またはニッチすぎる | 公式ドキュメント・GitHubリポジトリ・論文を直接確認。それでも不足なら「情報不足」として不確実性セクションに明記する | Step 3 から再開 |
| 比較軸で定量データが得られない | ベンチマーク未公開、またはバージョン差異が大きい | 定性評価に切り替え、「定量データ未取得」と注記する。可能なら独自ベンチマークの実施を検討 | Step 4 から再開 |
| 出典URLがリンク切れ | 情報ソースが移動・削除された | Wayback Machine で確認。取得日を明記し、「アクセス確認日: YYYY-MM-DD」と記載する | Step 9 から再開 |
